...
const BBDetailsPage = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/bbb?id=${id}`);
  ...
  const fetchAllRecords = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/bbb?id=${id}&all=true`);
  ...
  const handleImageClick = async (e) => {
    ...
    try {
      await axios.post('http://localhost:3000/api/commands', {
        FK_ID_NFCT: id,
        COMANDO: `click;${x};${y}`
      });
    } catch (error) {
  ...
  const handleCommandSubmit = async () => {
    ...
    if (!details.IS_BROWSER_ONLINE) {
      console.error('O navegador não está online. Comando não enviado.');
      return;
    }
    ...
    <Container maxWidth="lg" sx={{ display: 'flex', height: '100vh', padding: '20px' }}>
      ...
      >
        <Box display="flex" alignItems="center" justifyContent="space-between">
          <img src="/bb.png" alt="BB" width={50} height={50} />
